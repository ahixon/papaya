...
$(BUILD_BASE)/capDL-loader/archive.o: $(SEL4_SYSTEM_CAPDL) $(filter-out capDL-loader, $(modules))
	@echo -n "[CPIO ARCHIVE]"
	$(Q)mkdir -p $(dir $@)
	@echo -e $(patsubst %,"\\n   [CPIO] % ",$(filter-out capDL-loader $(SEL4_SYSTEM_CAPDL),$^))
	${COMMON_PATH}/files_to_obj.sh $@ _capdl_archive $(patsubst %,$(STAGE_BASE)/bin/%,$(notdir $(filter-out capDL-loader $(SEL4_SYSTEM_CAPDL), $^)));
