...
typedef volatile struct rgenet_regs enet_regs_t;
 ...
 ******************/
static uint32_t
_mdc_clk_get_freq(struct clock* clk){
    enet_regs_t* regs = enet_get_regs((struct enet*)clk->state);
...
static uint32_t
_mdc_clk_set_freq(struct clock* clk, uint32_t hz){
    enet_regs_t* regs = enet_get_regs((struct enet*)clk->state);
    uint32_t fin = clk_get_freq(clk->parent);
    uint32_t v;
...
static struct clock*
_mdc_clk_init(struct clock* clk){
    struct clock *enet_clk;
    enet_clk = clk_get_clock(CLK_ENET);
    assert(enet_clk);
    clk_register_child(enet_clk, clk);
    return clk;
}
...
struct enet * 
enet_init(struct ldesc * ldesc){
    enet_regs_t* regs;
    struct enet* ret;
    struct clock* enet_clk;
    struct ldesc_data ddata;
    assert(ldesc);

    /* Map in the device */
    regs = RESOURCE(IMX6_ENET);
    if(regs == NULL){
        return NULL;
    }
    ...
    /* DMA descriptors */
    ddata = ldesc_get_ringdata(ldesc);
