 ...
 * @return On success; return the handler ID associated with the connection.
 *         Otherwise, negative number to indicate error from svc_net.
 */
int rpc_new_udp(const struct ip_addr* server, int remote_port, 
 ...
 * @param[in] handler_id    ID for the connection with svc_net
 ...
 */
enum rpc_stat rpc_send(struct pbuf *pbuf, int len, int handler_id, 
 ...
 *                          and appropriate error is returned.
 */
enum rpc_stat rpc_call(struct pbuf *pbuf, int len, int handler_id, 
                       rpc_cb_fn func, void *callback, uintptr_t token);
