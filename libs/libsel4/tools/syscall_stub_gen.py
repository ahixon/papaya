...
#
#   * The script calls the function seL4_Call(), instead of emitting inline
#     assembly to perform the system call itself.
#
#     Performing the system call has the potential to generate much better
#     code; parameters to these stubs can be placed directly into the correct
#     register instead of bouncing in and out of the IPC buffer. ARM in
#     particular would benefit greatly from this.
#
#     This has not been done primarily because the current work focus is on
#     IA32. As Intel benevolently grants us 6 general purpose registers (of
#     which one is needed for a stack pointer, one is needed as a link
#     register, one is needed for the system call number, one is needed for the
#     destination cap, leaving only 64 bits for the actual message), the
#     benefit of directly emitting assembly is somewhat minimal.
#
#     (Why, oh why can't IA32 die?)
#
...
def generate_stub(arch, interface_name, method_name, method_id, input_params, output_params, registers):
    result = []
    ...
    result.append("\toutput_tag = __seL4_CallWithMRs(%s, tag," % (service_cap))
...
def generate_stub_file(arch, input_files, output_file):
    """
...
""")
    ...
    for (interface_name, method_name, method_id, inputs, outputs) in methods:
        result.append(generate_stub(arch, interface_name, method_name,
                method_id, inputs, outputs, registers))
...
def main():
    ...
    # Generate the stubs.
    generate_stub_file(options.arch, input_files, options.output)
